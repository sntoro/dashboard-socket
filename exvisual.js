...
setInterval(function () {
  ...
  sql.connect(db_config, (err) => {
    ...
    new sql.Request().query(
      ...
      ON CON.CHR_CONTAINER_CODE = CHR_ADD_CODE AND CON.CHR_NOPO_FAMILY = UP.CHR_NOPO_FAMILY WHERE CHR_CUST_NAME_ALIAS = '${customer}' AND LEFT(CHR_DATE_DELIVERY,6) = '${period}' AND CHR_PREPARE_STATUS = 1 \n\
      GROUP BY INT_QTY_CONTAINER_MAX, CON.CHR_CONTAINER_CODE ORDER BY CHR_CONTAINER_CODE`,
      (err, result) => {
        if (err) {
          console.log(err);
          return;
    ...
    new sql.Request().query(
      ...
      FROM TT_PACKING_UPLOAD A LEFT JOIN (SELECT COUNT(INT_NOPALLET) AS PALL, CHR_ADD_CODE  \n\
      FROM TT_PACKING_UPLOAD WHERE CHR_STAT = '1' AND CHR_CUST_NAME_ALIAS = '${customer}' AND LEFT(CHR_DATE_DELIVERY,6) = '${period}' \n\
      GROUP BY CHR_ADD_CODE) C ON A.CHR_ADD_CODE = C.CHR_ADD_CODE \n\
      WHERE CHR_CUST_NAME_ALIAS = '${customer}' AND LEFT(CHR_DATE_DELIVERY,6) = '${period}'\n\
    ...
    new sql.Request().query(
      `;WITH CTE ( CHR_NOPO_FAMILY, CHR_ADD_CODE, COUNT_PALLET, CHR_CUST_NAME_ALIAS)  AS ( \n\
        ...
        WHERE CHR_STAT = '1' AND CHR_CUST_NAME_ALIAS = ${customer}  \n\
        AND CHR_IDPALLET IN ( \n\
          ...
          AND X.CHR_BARCODE IN ( \n\
            ...
            FROM TT_PACKING_UPLOAD WHERE CHR_CUST_NAME_ALIAS = ${customer} \n\
      ...
      CTE_PACKING_UPLOAD ( CHR_NOPO_FAMILY, CHR_ADD_CODE, COUNT_PALLET, CHR_CUST_NAME_ALIAS ) AS ( \n\
        ...
        WHERE CHR_CUST_NAME_ALIAS = ${customer} \n\
    ...
    new sql.Request().query(
      ...
      WHERE FAM.CHR_CUST_NAME_ALIAS = ${customer} AND CHR_PREPARE_STATUS = 1\n\
    ...
    new sql.Request().query(
      ...
      WHERE CHR_STAT = '1' AND CHR_CUST_NAME_ALIAS = ${customer} GROUP BY CHR_CUST_NAME_ALIAS) C  \n\
      ON A.CHR_CUST_NAME_ALIAS = C.CHR_CUST_NAME_ALIAS \n\
      WHERE A.CHR_CUST_NAME_ALIAS = '${customer}' GROUP BY C.PALL`,
      (err, result) => {
        if (err) {
          console.log(err);
          return;
